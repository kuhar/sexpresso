...
auto Sexp::addChild(Sexp sexp) -> void {
  ...
  this->value.sexp.emplace_back(std::move(sexp));
}
...
static auto findChild(Sexp &sexp, std::string name) -> Sexp * {
  ...
  auto loc =
      std::find_if(sexp.value.sexp.begin(), sexp.value.sexp.end(), findPred);
...
auto Sexp::getChild(size_t idx) -> Sexp & { return this->value.sexp[idx]; }
...
static auto toStringImpl(Sexp const &sexp, std::ostringstream &ostream)
    -> void {
  switch (sexp.kind) {
  case SexpValueKind::STRING:
...
static auto childrenEqual(std::vector<Sexp> const &a,
                          std::vector<Sexp> const &b) -> bool {
  if (a.size() != b.size())
    return false;
...
auto Sexp::arguments() -> SexpArgumentRange { return SexpArgumentRange{*this}; }
...
auto SexpArgumentRange::end() -> iterator {
...
auto SexpArgumentRange::end() const -> const_iterator {
  return this->sexp.value.sexp.end();
}
